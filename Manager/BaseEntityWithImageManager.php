...
class BaseEntityWithImageManager extends BaseEntityWithFileManager
{
   ...
   private function transformPathWithFormat($path, $format){
    ...
    {
        $format = $format ? $format : $this->currentFormat;
        if($format){
        ...
        if($dim['size'] > 0){
            if($dim['crop']){
                ...
                if($dim['enlarge']){
                    ...
                    if ($layer->getWidth() > $dim['size'] || $layer->getHeight() > $dim['size']) {
                        $layer->resizeInPixel($dim['size'], $dim['size'], $dim['keep_proportions'], 0, 0, 'MM');
                    }
                }
            }
    ...
    public function removeFiles(BaseEntityWithFile $entity, $properties = array(), $doEraseFiles = true, $doSave = true, $doSetToNull = true)
    {
        ...
        foreach ($properties as $propertyName) {
            foreach ($this->imageFormatChoices[$propertyName] as $format) {
                ...
                parent::removeFiles($entity, $propertyName, $doEraseFiles, false, false);
